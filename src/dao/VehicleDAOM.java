		...
		try{
			...
			List<Document> foundDocument = collection.find().into(new ArrayList<Document>());
				for (int i = 0; i < foundDocument.size(); i++) {
					...
					//get accessory
					ArrayList<Document> acFount = new ArrayList<Document>();
					acFount = ((ArrayList<Document>) cols.get("accessory")); 
					for (int k = 0; k < acFount.size(); k++) {
						Document colc = (Document) acFount.get(k);
						String a_id = (String) colc.get("accessory_id");
						int ida = Integer.parseInt(a_id);
						String aName = (String) colc.get("name");
						String descriptionA = (String) colc.get("description");
					...
					if("car".equals(typeVeh)) {
						//String door = (String) cols.get("doors");
						//Integer d = Integer.valueOf(door);
						//String limit = (String) cols.get("passenger_limit");
						//int passenger_limit = Integer.parseInt(limit);
						//Car car = new Car(id, license_plate, colD, mo, manufa, acc, mileage, manufacturer_year, active, d, passenger_limit);
						count++;
						//ls.add(car);
					}else {
						String len = (String) cols.get("length");
